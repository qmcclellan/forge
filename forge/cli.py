...
def resolve_template_dir(template: str, template_dir_override: str | None = None) -> Path:
    """Where a template's source actually lives.

    Extracted so the validate handler and create_project resolve it the same
    way. They previously did not need to agree, because the handler never
    looked at the template directory at all -- which is exactly how KS-0017
    happened.
    """
...
def template_ships_optional(template_dir: Path) -> tuple[bool, bool]:
    """Whether the template actually ships Docker and Jenkins sources.

    KS-0017: `template validate` rendered its throwaway smoke project with
    with_docker=True and with_jenkins=True unconditionally, so the receipt
    claimed both were enabled even for a template that ships neither file. The
    receipt is discarded with the temporary directory, so nothing downstream
    was corrupted -- but a receipt that misreports what it just built is not a
    receipt, and the same call is the model for callers that do keep it.

    Reported from the template's own contents rather than from a flag, because
    the flag was the thing that was wrong.
    """
    return (
        (template_dir / "Dockerfile.tmpl").is_file(),
        (template_dir / "Jenkinsfile.tmpl").is_file(),
    )
...
def create_project(
    ...
    template_dir = resolve_template_dir(template, template_dir_override)
    ...
    if args.command == "template" and args.template_command == "validate":
        ...
        if not args.skip_smoke:
            with tempfile.TemporaryDirectory(prefix="forge-template-validate-") as temp_dir:
                smoke_docker, smoke_jenkins = template_ships_optional(
                    resolve_template_dir(args.template, args.template_dir)
                )
                target = create_project(
                    ...
                    with_docker=smoke_docker,
                    with_jenkins=smoke_jenkins,
